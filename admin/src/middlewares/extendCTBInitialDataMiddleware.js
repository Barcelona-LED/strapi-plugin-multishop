...
const extendCTBInitialDataMiddleware = () => {
  return () => next => action => {
    if (
      ...
      const multishop = { shopEnabled: false };

      const pluginOptions = action.data.pluginOptions
        ? { ...action.data.pluginOptions, multishop }
        : { multishop };
      ...
      // Override the action if the pluginOption config does not contain multishop
      // In this case we need to set the proper initialData shape
      if (!has(action.data.pluginOptions, 'multishop.shopEnabled')) {
        return next({ ...action, data });
      }
    }
