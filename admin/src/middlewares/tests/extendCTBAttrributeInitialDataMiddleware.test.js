...
describe('multishop | middlewares | extendCTBAttributeInitialDataMiddleware', () => {
  ...
  describe('should forward when the type is ContentTypeBuilder/FormModal/SET_ATTRIBUTE_DATA_SCHEMA', () => {
    ...
    it('should forward if the multishop is not activated is not contentType', () => {
      const middleware = extendCTBAttributeInitialDataMiddleware();
      const action = {
        forTarget: 'contentType',
        attributeType: 'text',
  ...
  it('should add the action.pluginOptions if the type is correct and multishop is activated', () => {
    const middleware = extendCTBAttributeInitialDataMiddleware();
    const action = {
      forTarget: 'contentType',
      attributeType: 'text',
    ...
    expect(next).toBeCalledWith({
      ...action,
      options: { pluginOptions: { multishop: { shopEnabled: true } } },
    });
  });
  ...
  it('should modify the options.pluginOptions when it exists', () => {
    ...
    expect(next).toBeCalledWith({
      ...action,
      options: { pluginOptions: { pluginTest: { ok: true }, multishop: { shopEnabled: true } } },
    });
  });
});
