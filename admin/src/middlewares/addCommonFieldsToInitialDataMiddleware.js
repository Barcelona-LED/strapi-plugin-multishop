...
const addCommonFieldsToInitialDataMiddleware = () => ({ getState, dispatch }) => next => action => {
  ...
  const relatedEntityId = get(query, 'plugins.multishop.relatedEntityId', null);
  const shop = get(query, 'plugins.multishop.shop', null);
  ...
  const getData = async () => {
    ...
    try {
      const requestURL = `/${pluginId}/content-manager/actions/get-non-shopEnabled-fields`;
      const body = { model: currentLayout.contentType.uid, id: relatedEntityId, shop };

      const data = await request(requestURL, { method: 'POST', body });

      const { nonShopEnabledFields, stores } = data;

      const merged = merge(defaultDataStructure, nonShopEnabledFields);
